...
const RUN_COLUMNS := [1, 2, 3, 4]  # walk_contact -> run_recovery
const IDLE_FPS := 4.0
const RUN_FPS := 10.0
...
func _initialize() -> void:
	...
	print("  animations: %d  (idle + run for %d directions)" % [frames.get_animation_names().size(), DIRECTIONS.size()])
...
func _pivot_offsets(pivots: Dictionary, baseline: int) -> Dictionary:
	...
	for dir_index in DIRECTIONS.size():
		...
		for i in RUN_COLUMNS.size():
			var col: int = RUN_COLUMNS[i]
			var off := _offset_for(pivots, _frame_key(dir_index, dir_name, col), baseline)
			map["run_%s/%d" % [dir_name, i]] = off
			if off != 0:
				corrected += 1
